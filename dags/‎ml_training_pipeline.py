...
with DAG(
    ...
    download_data = DockerOperator(
        task_id="download_data", command="uv run dvc repro download", **docker_kwargs
    )

    prepare_data = DockerOperator(
        task_id="prepare_data", command="uv run dvc repro prepare", **docker_kwargs
    )

    test_data = DockerOperator(
        task_id="test_data", command="uv run dvc repro test_data", **docker_kwargs
    )

    train_model = DockerOperator(
        task_id="train_model", command="uv run dvc repro train", **docker_kwargs
    )

    test_model = DockerOperator(
        task_id="test_model", command="uv run dvc repro test_model", **docker_kwargs
    )
