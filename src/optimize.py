...
def suggest_params(trial: optuna.Trial, cfg: DictConfig) -> dict:
    ...
    for param_name, config_vals in space.items():
        if cfg.hpo.sampler == "grid":
            params[param_name] = trial.suggest_categorical(param_name, list(config_vals))
        else:
            if isinstance(config_vals, (dict, DictConfig)) and 'low' in config_vals and 'high' in config_vals:
                if isinstance(config_vals.low, float) or isinstance(config_vals.high, float):
                    log_scale = config_vals.get("log", False)
                    params[param_name] = trial.suggest_float(
                        param_name, float(config_vals.low), float(config_vals.high), log=log_scale
                    )
                else:
...
def main(cfg: DictConfig) -> None:
    ...
    print(f"Запуск MLflow та Optuna ({cfg.model.type}) на {cfg.hpo.get('n_trials', 'всіх')} спроб...")
    ...
    with mlflow.start_run(run_name=parent_run_name):
        ...
        study = optuna.create_study(direction=cfg.hpo.get("direction", "minimize"), sampler=sampler) 
